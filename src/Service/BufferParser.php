...
class BufferParser extends AbstractController
{
    ...
    public function buildData(OutputInterface $output)
    {
        ...
        if ($parsingBufferFile->flock(LOCK_EX)) {
            ...
            while (true) {
                $line = $parsingBufferFile->fgets();
                $lastLine = $parsingBufferFile->eof();
                ...
                if ($numberOfParsedRows >= $this->getRowParserLimit()
                    || $lastLine
                ) {
                    $saveSuccess = $this->saveDataToStatistics($insertData);
                    //file_put_contents("BufferParserData.txt", serialize($insertData));
                    if ($saveSuccess) {
                        $insertData = [];
                        if ($lastLine) {
                            $output->writeln("Trying to remove temp buffer \n");
                            $parsingBufferFile->flock(LOCK_UN);
                            $parsingBufferFile = null;
                            $this->mainBuffer->removeTempBuffer();
                            break;
                        } else {
                            $output->writeln(
                                "Trying to remove first %d lines from buffer \n",
                                $this->getRowParserLimit()
                            );
                            $this->mainBuffer->removeFirstNLinesFromFile($parsingBufferFile, $this->getRowParserLimit());
                        }
                        $numberOfParsedRows = 0;
                    } else {
                        $this->logger->error("Failed to save parser rows to database");
                        break;
                    }
                }
            }
            ...
            $this->logger->error("Unable to lock file", 'App\Service\BufferParser');
